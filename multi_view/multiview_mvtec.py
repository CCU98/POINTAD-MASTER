...
def get_mv_images(tiff_path, rgb_path, gt_path, point_size, view_save_path = "./mutilv_o", gt_save_path = "./mutilv_o",cor_save_path = './mutilv_o',file_id='000', rotation_matrix=None):
    ...
    # 如果提供了旋转矩阵，则在创建点云之前对点坐标应用旋转
    # 旋转矩阵应为 3x3 的 numpy 数组；按惯例将点 P 变换为 P_rot = P @ R.T
    if rotation_matrix is not None:
        try:
            rot = np.asarray(rotation_matrix, dtype=np.float64)
            if rot.shape == (3, 3):
                unorganized_pc = (unorganized_pc @ rot.T)
        except Exception:
            pass
...
# python multi_view/multiview_mvtec.py --data_path ./data/visa