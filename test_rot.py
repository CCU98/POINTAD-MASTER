import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
import AnomalyCLIP_lib
import torch
import argparse
import torch.nn.functional as F
from prompt_ensemble import AnomalyCLIP_PromptLearner
from loss import FocalLoss, BinaryDiceLoss
from utils import normalize
from dataset import Dataset, generate_is_seen_for_point_in_each_view
from logger import get_logger
from tqdm import tqdm

# import os
# os.environ["CUDA_VISIBLE_DEVICES"] = "1"
import random
import numpy as np
from tabulate import tabulate
from utils import get_transform

def setup_seed(seed):
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    np.random.seed(seed)
    random.seed(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False



from visualization import visualizer

from metrics import image_level_metrics, pixel_level_metrics
from tqdm import tqdm
from scipy.ndimage import gaussian_filter


def back_to_3d(d2_similarity_map, d2_3d_cor, non_zero_index, ori_resolution = 336):
    # _, h, w, _ = d2_similarity_map.shape
    h = torch.sqrt(torch.tensor(d2_3d_cor.shape[2])).int()
    w = h
    b, nv, num_points, _ = d2_3d_cor.shape
    xx = d2_3d_cor[:, :, :, 0].reshape(-1).long()
    yy = d2_3d_cor[:, :, :, 1].reshape(-1).long()
    nbatch = torch.repeat_interleave(torch.arange(0, b*nv)[:,None], num_points).reshape(-1, ).cuda().long()
    d2_similarity_map = d2_similarity_map.permute(0, 3, 1, 2)
    point_logits = d2_similarity_map[nbatch, :, yy, xx]
    point_logits = point_logits.reshape(b, nv, num_points, 2)
    vweights = torch.ones((1, nv, 1, 1))
    vweights = vweights.reshape(1, -1, 1, 1).to(point_logits.device)
    is_seen = d2_3d_cor[:, :, :, 2].reshape(b, nv, num_points, 1)
    point_logits = point_logits * vweights * is_seen * non_zero_index
    mask = is_seen.bool() | (~non_zero_index.bool())
    point_logits = point_logits.sum(1)/ (mask.sum(1))
    point_logits = point_logits.reshape(b, h, w, 2)
    return point_logits


def _find_rotated_dir(orig_dir, rot_suffix, rot_tag=None):
    """Try to find rotated directory corresponding to orig_dir.
    Strategy: replace parent '2d_rendering' with '2d_rendering_{rot_suffix}' and
    try inserting rot_tag as an intermediate folder when provided.
    """
    if orig_dir is None:
        return None
    basename = os.path.basename(orig_dir)
    parent = os.path.dirname(orig_dir)
    grandparent = os.path.dirname(parent)
    candidate_base = os.path.join(grandparent, f'2d_rendering_{rot_suffix}')
    # try candidate_base/rot_tag/basename
    if rot_tag is not None:
        p = os.path.join(candidate_base, rot_tag, basename)
        if os.path.isdir(p):
            return p
    # try candidate_base/basename
    p = os.path.join(candidate_base, basename)
    if os.path.isdir(p):
        return p
    # fallback: simple replace '2d_rendering' -> '2d_rendering_{rot_suffix}'
    if '2d_rendering' in orig_dir:
        return orig_dir.replace('2d_rendering', f'2d_rendering_{rot_suffix}')
    # last resort: try replace 'xyz' with '2d_rendering_{rot_suffix}' in ancestor
    if 'xyz' in orig_dir:
        return orig_dir.replace('xyz', f'2d_rendering_{rot_suffix}')
    return None


def _find_rotated_generic(orig_dir, rot_suffix, key_name, rot_tag=None):
    # key_name examples: '2d_gt', '2d_3d_cor'
    if orig_dir is None:
        return None
    basename = os.path.basename(orig_dir)
    parent = os.path.dirname(orig_dir)
    grandparent = os.path.dirname(parent)
    candidate_base = os.path.join(grandparent, f'{key_name}_{rot_suffix}')
    if rot_tag is not None:
        p = os.path.join(candidate_base, rot_tag, basename)
        if os.path.isdir(p):
            return p
    p = os.path.join(candidate_base, basename)
    if os.path.isdir(p):
        return p
    if key_name in orig_dir:
        return orig_dir.replace(key_name, f'{key_name}_{rot_suffix}')
    return None


def _reload_rotated_views(orig_img_dir, orig_gt_dir, orig_cor_dir, rot_suffix, rot_tag, transform, target_transform, point_size):
    # locate rotated dirs
    img_dir = _find_rotated_dir(orig_img_dir, rot_suffix, rot_tag)
    gt_dir = _find_rotated_generic(orig_gt_dir, rot_suffix, '2d_gt', rot_tag)
    cor_dir = _find_rotated_generic(orig_cor_dir, rot_suffix, '2d_3d_cor', rot_tag)
    if img_dir is None or gt_dir is None or cor_dir is None:
        raise FileNotFoundError(f"Cannot find rotated dirs for {orig_img_dir} with suffix {rot_suffix} tag {rot_tag}")

    # load render images
    d2_render_img_list = []
    for filename in sorted(os.listdir(img_dir)):
        from PIL import Image
        img = Image.open(os.path.join(img_dir, filename)).convert('RGB')
        img = transform(img) if transform is not None else img
        d2_render_img_list.append(img)
    d2_render_img = torch.stack(d2_render_img_list)

    # load gt masks and anomalies
    d2_render_gt_list = []
    rendering_anomaly_list = []
    for filename in sorted(os.listdir(gt_dir)):
        from PIL import Image
        img_mask = Image.open((os.path.join(gt_dir, filename))).convert('L')
        img_mask = target_transform(img_mask)
        img_mask[img_mask>0.5] = 1.0
        img_mask[img_mask<=0.5] = 0.0
        rendering_anomaly = 0.0 if torch.all(img_mask == 0) else 1.0
        d2_render_gt_list.append(img_mask)
        rendering_anomaly_list.append(rendering_anomaly)
    d2_render_gt = torch.stack(d2_render_gt_list)
    rendering_anomaly = torch.tensor(rendering_anomaly_list)

    # load correspondence npy files
    d2_3d_cor_list = []
    non_zero_index_arr = None
    for idx, filename in enumerate(sorted(os.listdir(cor_dir))):
        fullp = os.path.join(cor_dir, filename)
        if idx == 0:
            template_non_zero_index = torch.zeros(point_size * point_size, dtype = torch.long)
            non_zero_index = np.load(fullp)
            non_zero_index = torch.from_numpy(non_zero_index)
            template_non_zero_index[non_zero_index] = 1
            non_zero_index_arr = template_non_zero_index.reshape(-1, 1)
        else:
            template_d2_corrdinate = torch.zeros(point_size * point_size, 3, dtype = torch.long)
            d2_corrdinate = np.load(fullp)
            d2_corrdinate = torch.from_numpy(d2_corrdinate).long()
            template_d2_corrdinate[non_zero_index] = d2_corrdinate
            d2_3d_cor_list.append(template_d2_corrdinate)

    # 在 Dataset 中会对 d2_3d_cor_list 进行可见性修正，复用相同逻辑
    is_seen, d2_3d_cor = generate_is_seen_for_point_in_each_view(d2_3d_cor_list, non_zero_index_arr)
    return d2_render_img, rendering_anomaly, d2_render_gt, d2_3d_cor, non_zero_index_arr

def test(args):
    img_size = args.image_size
    features_list = args.features_list
    dataset_dir = args.data_path
    save_path = args.save_path
    dataset_name = args.dataset
    train_class = args.train_class

    logger = get_logger(args.save_path)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    AnomalyCLIP_parameters = {"Prompt_length": args.n_ctx, "learnabel_text_embedding_depth": args.depth, "learnabel_text_embedding_length": args.t_n_ctx}
    
    model, _ = AnomalyCLIP_lib.load("ViT-L/14@336px", device=device, design_details = AnomalyCLIP_parameters)
    model.eval()

    preprocess, target_transform, target_transform_pc = get_transform(args)
    print(dataset_dir)
    test_data = Dataset(root=dataset_dir, dataset_name = args.dataset, transform=preprocess, target_transform=target_transform, target_transform_pc = target_transform_pc, mode='test', is_all = True, point_size = args.point_size)
    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=1, shuffle=False)
    obj_list = test_data.obj_list
    # 如果通过命令行指定了 test_class，则只保留该类别
    if getattr(args, 'test_class', None):
        if args.test_class not in obj_list:
            raise ValueError(f"Requested test_class '{args.test_class}' not found in dataset classes: {obj_list}")
        if args.test_class == train_class:
            print(f"Warning: requested test_class '{args.test_class}' equals train_class '{train_class}'. Proceeding anyway.")
        obj_list = [args.test_class]
    results = {}
    # metrics = {}
    for obj in obj_list:
        results[obj] = {}
        results[obj]['gt_sp'] = []
        results[obj]['pr_sp'] = []
        results[obj]['color_pr_sp'] = []
        results[obj]['integrate_pr_sp'] = []

        results[obj]['imgs_masks'] = []
        results[obj]['anomaly_maps'] = []
        results[obj]['color_anomaly_maps'] = []
        results[obj]['integrate_anomaly_maps'] = []

        # metrics[obj] = {}
        # metrics[obj]['pixel-auroc'] = 0
        # metrics[obj]['pixel-aupro'] = 0
        # metrics[obj]['image-auroc'] = 0
        # metrics[obj]['image-ap'] = 0
    print("Testing on classes:", obj_list)
    obj_list = [c for c in obj_list if c != args.train_class]
    print("Testing on classes:", obj_list)
    prompt_learner = AnomalyCLIP_PromptLearner(model.to("cpu"), AnomalyCLIP_parameters)
    checkpoint = torch.load(args.checkpoint_path)
    prompt_learner.load_state_dict(checkpoint["prompt_learner"])
    prompt_learner.to(device)
    model.to(device)
    model.visual.DAPM_replace(DPAM_layer = 20)

    prompts, tokenized_prompts, compound_prompts_text = prompt_learner(cls_id = None)
    text_features = model.encode_text_learn(prompts, tokenized_prompts, compound_prompts_text).float()
    text_features = torch.stack(torch.chunk(text_features, dim = 0, chunks = 2), dim = 1)
    text_features = text_features/text_features.norm(dim=-1, keepdim=True)


    model.to(device)
    for idx, items in enumerate(tqdm(test_dataloader)):
        image = items['img'].to(device)
        cls_name = items['cls_name']
        cls_id = items['cls_id']
        # 如果该样本的类别不在我们准备的 results（例如使用 --test_class 限定），则跳过
        if cls_name[0] not in results:
            # 可选：打印调试信息，但避免过多输出
            # print(f"Skipping sample for class {cls_name[0]} (not in target test set)")
            continue
        gt_mask = items['img_mask']
        gt_mask[gt_mask > 0.5], gt_mask[gt_mask <= 0.5] = 1, 0
        results[cls_name[0]]['imgs_masks'].append(gt_mask)  # px
        results[cls_name[0]]['gt_sp'].extend(items['anomaly'].detach().cpu())


        # 如果指定了旋转后渲染的后缀，则尝试从旋转目录重新加载视图/gt/对应文件
        if getattr(args, 'rot_suffix', None):
            # 增加 [0] 来获取实际的路径字符串
            orig_img_dir = items.get('d2_render_img_path', None)
            if isinstance(orig_img_dir, list):
                orig_img_dir = orig_img_dir[0]
        # if getattr(args, 'rot_suffix', None):
        #     orig_img_dir = items.get('d2_render_img_path', None)
            # derive original gt and cor dirs by convention
            orig_gt_dir = None
            orig_cor_dir = None
            if orig_img_dir is not None:
                orig_gt_dir = orig_img_dir.replace('2d_rendering', '2d_gt')
                orig_cor_dir = orig_img_dir.replace('2d_rendering', '2d_3d_cor')
            try:
                render_img_t, rendering_anomaly, render_gt_t, d2_3d_cor_t, non_zero_index_arr = _reload_rotated_views(orig_img_dir, orig_gt_dir, orig_cor_dir, args.rot_suffix, args.rot_tag, preprocess, target_transform, args.point_size)
                # shapes: render_img_t: nv x c x h x w
                b = 1
                nv, c, h, w = render_img_t.shape
                render_image = render_img_t.reshape(-1, c, h, w).to(device)
                d2_render_anomaly = rendering_anomaly.unsqueeze(0).to(device)
                d2_3d_cor = d2_3d_cor_t.unsqueeze(0).to(device)
                non_zero_index = non_zero_index_arr.unsqueeze(0).to(device)
                non_zero_index = non_zero_index.unsqueeze(1).repeat(1, nv, 1, 1)
            except Exception as e:
                print(f"Failed to load rotated views for {orig_img_dir} rot={getattr(args,'rot_tag',None)}: {e}")
                # fallback to original loaded tensors
                render_image = items['d2_render_img'].to(device)
                b, nv, c, h, w = render_image.shape
                render_image = render_image.reshape(-1, c, h, w)
                d2_render_anomaly = items['d2_render_anomaly'].to(device)
                d2_3d_cor = items['d2_3d_cor'].to(device)
                non_zero_index = items['non_zero_index'].to(device)
                non_zero_index = non_zero_index.unsqueeze(1).repeat(1, nv, 1, 1)
        else:
            render_image = items['d2_render_img'].to(device)
            b, nv, c, h, w = render_image.shape
            render_image = render_image.reshape(-1, c, h, w)
            d2_render_anomaly = items['d2_render_anomaly'].to(device)
            d2_3d_cor = items['d2_3d_cor'].to(device)
            non_zero_index = items['non_zero_index'].to(device)
            non_zero_index = non_zero_index.unsqueeze(1).repeat(1, nv, 1, 1)

############################################################ 考虑color
        whole_image = torch.cat([render_image, image], dim = 0)
############################################################

        with torch.no_grad():
            image_features, patch_features = model.encode_image(whole_image, features_list, DPAM_layer = 20)
            image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            text_probs = image_features.unsqueeze(1) @ text_features.permute(0, 2, 1)
            text_probs = (text_probs/0.07).softmax(-1)
            text_probs = text_probs[:, 0, 1]
####################################################
            color_text_probs = text_probs[-b:]
            text_probs = text_probs[:-b]
            text_probs = torch.chunk(text_probs, nv, dim = 0)
            text_probs = torch.stack(text_probs, dim = 1).mean(1)
####################################################
            patch_feature = patch_features[0]
            patch_feature = patch_feature/ patch_feature.norm(dim = -1, keepdim = True)
            similarity, _ = AnomalyCLIP_lib.compute_similarity(patch_feature, text_features[0])
            similarity_map = AnomalyCLIP_lib.get_similarity_map(similarity[:, 1:, :], args.image_size)
####################################################
            color_similarity_map = similarity_map[-b:]
            similarity_map = similarity_map[:-b]
####################################################
            similarity_map_nv = torch.chunk(similarity_map, nv, dim = 0)

            similarity_map_nv = torch.stack(similarity_map_nv, dim = 1)
            anomaly_map_nv = (similarity_map_nv[...,1] + 1 - similarity_map_nv[...,0])/2.0

            # print("text_probs similarity_map_nv anomaly_map_nv", text_probs.shape, similarity_map_nv.shape, anomaly_map_nv.shape)
            #########################################
            d3_similarity_map = back_to_3d(similarity_map, d2_3d_cor, non_zero_index)
            ####################################################
            anomaly_map = d3_similarity_map[...,1]
            color_map = (color_similarity_map[...,1] + 1 - color_similarity_map[...,0])/2.0
            ####################################################
            
            
            anomaly_map = torch.stack([torch.from_numpy(gaussian_filter(i, sigma = args.sigma)) for i in anomaly_map.detach().cpu()], dim = 0)
            color_anomaly_map = torch.stack([torch.from_numpy(gaussian_filter(i, sigma = args.sigma)) for i in color_map.detach().cpu()], dim = 0)
            
            integrate_anomaly_map = (color_anomaly_map + anomaly_map)/2
            integrate_anomaly_map = torch.stack([torch.from_numpy(gaussian_filter(i, sigma = args.sigma)) for i in integrate_anomaly_map.detach().cpu()], dim = 0)
#####################################################################

            results[cls_name[0]]['pr_sp'].extend(0.5*anomaly_map.max() + 0.5*text_probs.detach().cpu())
            results[cls_name[0]]['color_pr_sp'].extend(color_anomaly_map.max() + color_text_probs.detach().cpu())
            results[cls_name[0]]['integrate_pr_sp'].extend((integrate_anomaly_map.max() + (color_text_probs + text_probs)/2).detach().cpu())
            results[cls_name[0]]['anomaly_maps'].append(anomaly_map)
            results[cls_name[0]]['color_anomaly_maps'].append(color_anomaly_map)
            results[cls_name[0]]['integrate_anomaly_maps'].append(integrate_anomaly_map)


    table_ls = []
    image_auroc_list = []
    image_ap_list = []
    pixel_auroc_list = []
    pixel_aupro_list = []

    color_table_ls = []
    color_image_auroc_list = []
    color_image_ap_list = []
    color_pixel_auroc_list = []
    color_pixel_aupro_list = []

    integrate_table_ls = []
    integrate_image_auroc_list = []
    integrate_image_ap_list = []
    integrate_pixel_auroc_list = []
    integrate_pixel_aupro_list = []

    # obj_list = [c for c in test_data.obj_list if c != train_class]
    for obj in obj_list:
        table = []
        color_table = []
        integrate_table = []
        table.append(obj)
        color_table.append(obj)
        integrate_table.append(obj)
        results[obj]['imgs_masks'] = torch.cat(results[obj]['imgs_masks'])
        results[obj]['anomaly_maps'] = torch.cat(results[obj]['anomaly_maps']).detach().cpu().numpy()
        results[obj]['color_anomaly_maps'] = torch.cat(results[obj]['color_anomaly_maps']).detach().cpu().numpy()
        results[obj]['integrate_anomaly_maps'] = torch.cat(results[obj]['integrate_anomaly_maps']).detach().cpu().numpy()
        if args.metrics == 'image-level':
            #### point
            image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'pr_sp')
            image_ap = image_level_metrics(results, obj, "image-ap", modality = 'pr_sp')
            table.append(str(np.round(image_auroc * 100, decimals=1)))
            table.append(str(np.round(image_ap * 100, decimals=1)))
            image_auroc_list.append(image_auroc)
            image_ap_list.append(image_ap)
            #### color
            color_image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'color_pr_sp')
            color_image_ap = image_level_metrics(results, obj, "image-ap", modality = 'color_pr_sp')
            color_table.append(str(np.round(color_image_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_image_ap * 100, decimals=1)))
            color_image_auroc_list.append(color_image_auroc)
            color_image_ap_list.append(color_image_ap) 
            #### point + color
            integrate_image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'integrate_pr_sp')
            integrate_image_ap = image_level_metrics(results, obj, "image-ap", modality = 'integrate_pr_sp')
            integrate_table.append(str(np.round(integrate_image_auroc * 100, decimals=1)))
            integrate_table.append(str(np.round(integrate_image_ap * 100, decimals=1)))
            integrate_image_auroc_list.append(integrate_image_auroc)
            integrate_image_ap_list.append(integrate_image_ap)  
        elif args.metrics == 'pixel-level':
            #### point
            pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'anomaly_maps')
            pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'anomaly_maps')
            table.append(str(np.round(pixel_auroc * 100, decimals=1)))
            table.append(str(np.round(pixel_aupro * 100, decimals=1)))
            pixel_auroc_list.append(pixel_auroc)
            pixel_aupro_list.append(pixel_aupro)
            #### color
            color_pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'color_anomaly_maps')
            color_pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'color_anomaly_maps')
            color_table.append(str(np.round(color_pixel_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_pixel_aupro * 100, decimals=1)))
            color_pixel_auroc_list.append(color_pixel_auroc)
            color_pixel_aupro_list.append(color_pixel_aupro)
            #### point + color
            integrate_pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'integrate_anomaly_maps')
            integrate_pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'integrate_anomaly_maps')
            integrate_table.append(str(np.round(integrate_pixel_auroc * 100, decimals=1)))
            integrate_table.append(str(np.round(integrate_pixel_aupro * 100, decimals=1)))
            integrate_pixel_auroc_list.append(integrate_pixel_auroc)
            integrate_pixel_aupro_list.append(integrate_pixel_aupro)
        elif args.metrics == 'image-pixel-level':
            #### point
            image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'pr_sp')
            image_ap = image_level_metrics(results, obj, "image-ap", modality = 'pr_sp')
            pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'anomaly_maps')
            pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'anomaly_maps')
            table.append(str(np.round(pixel_auroc * 100, decimals=1)))
            table.append(str(np.round(pixel_aupro * 100, decimals=1)))
            table.append(str(np.round(image_auroc * 100, decimals=1)))
            table.append(str(np.round(image_ap * 100, decimals=1)))
            image_auroc_list.append(image_auroc)
            image_ap_list.append(image_ap) 
            pixel_auroc_list.append(pixel_auroc)
            pixel_aupro_list.append(pixel_aupro)
            #### color
            color_image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'color_pr_sp')
            color_image_ap = image_level_metrics(results, obj, "image-ap", modality = 'color_pr_sp')
            color_pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'color_anomaly_maps')
            color_pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'color_anomaly_maps')
            color_table.append(str(np.round(color_pixel_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_pixel_aupro * 100, decimals=1)))
            color_table.append(str(np.round(color_image_auroc * 100, decimals=1)))
            color_table.append(str(np.round(color_image_ap * 100, decimals=1)))
            color_image_auroc_list.append(color_image_auroc)
            color_image_ap_list.append(color_image_ap) 
            color_pixel_auroc_list.append(color_pixel_auroc)
            color_pixel_aupro_list.append(color_pixel_aupro)
            #### point + color
            integrate_image_auroc = image_level_metrics(results, obj, "image-auroc", modality = 'integrate_pr_sp')
            integrate_image_ap = image_level_metrics(results, obj, "image-ap", modality = 'integrate_pr_sp')
            integrate_pixel_auroc = pixel_level_metrics(results, obj, "pixel-auroc", modality = 'integrate_anomaly_maps')
            integrate_pixel_aupro = pixel_level_metrics(results, obj, "pixel-aupro", modality = 'integrate_anomaly_maps')
            integrate_table.append(str(np.round(integrate_pixel_auroc * 100, decimals=1)))
            integrate_table.append(str(np.round(integrate_pixel_aupro * 100, decimals=1)))
            integrate_table.append(str(np.round(integrate_image_auroc * 100, decimals=1)))
            integrate_table.append(str(np.round(integrate_image_ap * 100, decimals=1)))
            integrate_image_auroc_list.append(integrate_image_auroc)
            integrate_image_ap_list.append(integrate_image_ap) 
            integrate_pixel_auroc_list.append(integrate_pixel_auroc)
            integrate_pixel_aupro_list.append(integrate_pixel_aupro)
        table_ls.append(table)
        color_table_ls.append(color_table)
        integrate_table_ls.append(integrate_table)
    if args.metrics == 'image-level':
        # logger
        table_ls.append(['mean', 
                        str(np.round(np.mean(image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(image_ap_list) * 100, decimals=1))])
        results = tabulate(table_ls, headers=['objects', 'image_auroc', 'image_ap'], tablefmt="pipe")

        color_table_ls.append(['mean', 
                        str(np.round(np.mean(color_image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_image_ap_list) * 100, decimals=1))])

        color_results = tabulate(color_table_ls, headers=['objects', 'image_auroc', 'image_ap'], tablefmt="pipe")
    
        integrate_table_ls.append(['mean', 
                        str(np.round(np.mean(integrate_image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(integrate_image_ap_list) * 100, decimals=1))])


        integrate_results = tabulate(integrate_table_ls, headers=['objects', 'image_auroc', 'image_ap'], tablefmt="pipe")
    
    elif args.metrics == 'pixel-level':
        # logger
        table_ls.append(['mean', str(np.round(np.mean(pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(pixel_aupro_list) * 100, decimals=1))
                       ])
        results = tabulate(table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro'], tablefmt="pipe")

        color_table_ls.append(['mean', str(np.round(np.mean(color_pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_pixel_aupro_list) * 100, decimals=1)), 
                       ])

        color_results = tabulate(color_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro'], tablefmt="pipe")
    
        integrate_table_ls.append(['mean', str(np.round(np.mean(integrate_pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(integrate_pixel_aupro_list) * 100, decimals=1))
                        ])

        integrate_results = tabulate(integrate_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro'], tablefmt="pipe")
    
    elif args.metrics == 'image-pixel-level':
        # logger
        table_ls.append(['mean', str(np.round(np.mean(pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(pixel_aupro_list) * 100, decimals=1)), 
                        str(np.round(np.mean(image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(image_ap_list) * 100, decimals=1))])
        results = tabulate(table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro', 'image_auroc', 'image_ap'], tablefmt="pipe")
    
        color_table_ls.append(['mean', str(np.round(np.mean(color_pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_pixel_aupro_list) * 100, decimals=1)), 
                        str(np.round(np.mean(color_image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(color_image_ap_list) * 100, decimals=1))])

        color_results = tabulate(color_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro', 'image_auroc', 'image_ap'], tablefmt="pipe")
    
        integrate_table_ls.append(['mean', str(np.round(np.mean(integrate_pixel_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(integrate_pixel_aupro_list) * 100, decimals=1)), 
                        str(np.round(np.mean(integrate_image_auroc_list) * 100, decimals=1)),
                        str(np.round(np.mean(integrate_image_ap_list) * 100, decimals=1))])


        integrate_results = tabulate(integrate_table_ls, headers=['objects', 'pixel_auroc', 'pixel_aupro', 'image_auroc', 'image_ap'], tablefmt="pipe")
    
    # Add a clear header when running rotated-tag experiments so outputs are
    # easy to distinguish when test() is invoked repeatedly (e.g. full_0/full_1/full_2).
    rot_tag = getattr(args, 'rot_tag', None)
    if rot_tag is not None:
        logger.info("\n=== Rotated views tag: %s ===", rot_tag)

    logger.info("\n[Point-only results]\n%s", results)

    logger.info("\n[Color-only results]\n%s", color_results)

    logger.info("\n[Integrated results]\n%s", integrate_results)

if __name__ == '__main__':
    parser = argparse.ArgumentParser("PointAD", add_help=True)
    # paths
    parser.add_argument("--data_path", type=str, default="./data/visa", help="path to test dataset")
    parser.add_argument("--save_path", type=str, default='./results/', help='path to save results')
    parser.add_argument("--checkpoint_path", type=str, default='./checkpoint/', help='path to checkpoint')
    # model
    parser.add_argument("--dataset", type=str, default='mvtec_pc_3d_rgb')
    parser.add_argument("--features_list", type=int, nargs="+", default=[6, 12, 18, 24], help="features used")
    parser.add_argument("--image_size", type=int, default=518, help="image size")
    parser.add_argument("--depth", type=int, default=9, help="image size")
    parser.add_argument("--n_ctx", type=int, default=12, help="zero shot")
    parser.add_argument("--t_n_ctx", type=int, default=4, help="zero shot")
    parser.add_argument("--feature_map_layer", type=int,  nargs="+", default=[0, 1, 2, 3], help="zero shot")
    parser.add_argument("--metrics", type=str, default='image-pixel-level')
    parser.add_argument("--seed", type=int, default=111, help="random seed")
    parser.add_argument("--sigma", type=int, default=4, help="zero shot")
    parser.add_argument("--train_class", type=str, default="cookie", help="zero shot or few shot")
    parser.add_argument("--point_size", type=int, default=336, help="save frequency")
    parser.add_argument('--rot_suffix', type=str, default=None, help='如果指定，则从旋转后的渲染目录读取视图（例如 rot_full）')
    parser.add_argument('--test_class', type=str, default=None, help='如果指定，则只对该类别运行测试（例如 dowel）')
    args = parser.parse_args()
    parser.add_argument('--rot_tag', type=str, default=None, help='如果指定，则只测试该 rot_tag（如 full_0）；否则若 rot_suffix 存在将依次测试 full_0/full_1/full_2')
    # parse again to pick up rot_tag if provided
    args = parser.parse_args()
    print(args)
    setup_seed(args.seed)
    # 如果指定了 rot_suffix 但未指定 rot_tag，就依次对 full_0/full_1/full_2 运行测试
    if args.rot_suffix and args.rot_tag is None:
        for tag in ['full_0', 'full_1', 'full_2']:
            print(f"\n=== Running test for rotated views: suffix={args.rot_suffix} tag={tag} ===\n")
            args.rot_tag = tag
            test(args)
    else:
        test(args)
